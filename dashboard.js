...
class FantasyDashboard {
    ...
    formatStatsPreview(stats) {
        ...
        // Complete stat mappings based on Yahoo Fantasy Football stat IDs
        ...
        return keyStats.length > 0 ? keyStats.slice(0, 6).join(', ') : 'Various stats available';
    }
    ...
    updatePagination() {
        ...
        if (this.elements.prevPageBtn) {
            this.elements.prevPageBtn.disabled = !this.pagination.hasPrev;
        }
        
        if (this.elements.nextPageBtn) {
            this.elements.nextPageBtn.disabled = !this.pagination.hasNext;
        }
   ...
   async clearStatsCache() {
       try {
           ...
           if (window.statsDB) {
               const key = window.statsDB.generateLoadKey(this.currentYear, this.currentWeek, this.currentPosition, this.currentPage);
...
document.addEventListener('DOMContentLoaded', () => {
   ...
   document.addEventListener('keydown', (e) => dashboard.handleStatsKeyboardShortcuts(e));
   ...
   console.log('🏈 Fantasy Dashboard initialized with auto-loading stats!');
   console.log('📊 Initial stats data will load automatically...');
